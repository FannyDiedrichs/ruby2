...
def combien_handle(journ)
  puts "Il y a #{journ.count} handle dans cette array."
  puts "\n"
end

#2. Quel est le handle le plus court de cette liste ?
def short_handle(journ)
  puts "Le handle le plus court est #{journ.min_by(&:length)}"
  puts "\n"
end

#3. Combien y-a-t'il de handle contenant 5 caractères (le @ ne compte pas pour un caractère)
def cinq_caract(journ)
   print "Le nombre de handles avec 5 caractère est "
   puts journ.count { |element| element.length == 6 }
   puts "\n"
end

#4. Combien commencent par une majuscule (première lettre juste après le @) ?
def nbr_debut_maj(journ)
   print "Le nombre de handles qui commencent par une majuscule est "
   puts journ.count {|element| element[1].match?( /[[:upper:]]/ )}
   puts "\n"
end

#5. Trie la liste de handle par ordre alphabétique.
def alpha_order(journ)
puts "Voici la liste par ordre alphabétique"
  puts journ.sort_by(&:downcase)
  puts "\n"
end

#6. Trie la liste de handle par taille des handle (les plus petits en premiers, les plus grands après)
def lenght_order(journ)
  puts "Voici la liste par ordre de taille de handle"
  print journ.sort_by! {|element| element.size}
  puts "\n"
end

#7. Quelle est la position dans l'array de la personne @epenser ?
def position(journ)
  puts "Le handle @epenser est en position #{journ.find_index("@epenser")}."
  puts "\n"
end
...
def order_taille(journ)
  repartition = journ.group_by(&:length)
  puts "\n"
end

def handle_repartition(journ)
  repartition = journ.group_by(&:length)
  tabvalue =[]
  repartition.each do |k,v|
  valeur = v.count
  tabvalue << valeur
  end

  tabkey = []
  repartition.each_key do |k|
    tabkey << k
  end

    hash = tabkey.zip(tabvalue).to_h 
   (hash.length).times do |i|
    puts " il y a #{hash.values[i]} handle qui a/ont #{hash.keys[i]-1} caractères"
   end
end
